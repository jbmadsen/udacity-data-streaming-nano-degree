...
def get_partitions(topic_name):
    ...
    # Using the above endpoints as an example, list partitions for a given topic name using the API
    ...
    try:
        ...
        print(f"ERror getting partitions. Response data: {json.dumps(resp.json(), indent=2)})")

    print("Got partitions data from Kafka:")
    print(json.dumps(resp.json(), indent=2))
