...
from pyspark.sql.types import StructField, StructType, StringType, FloatType, BooleanType, ArrayType, DateType, IntegerType

# TODO: create deposit a kafka message schema StructType including the following JSON elements:
...
# TODO: create a customer kafka message schema StructType including the following JSON elements:
...
spark = SparkSession.builder.appName("exercise4-customer-deposits").getOrCreate()
...
customer_deposits_stream = spark \
    .readStream \
    .format("kafka") \
...
customer_deposits_df = customer_deposits_stream.selectExpr("cast(key as string) key", "cast(value as string) value")
...
customer_deposits_df \
    .withColumn("value",from_json("value",depositKafkaMessageSchema)) \
    .select(col('value.*')) \
    .createOrReplaceTempView("BankDeposits")

# TODO: using spark.sql, select * from BankDeposits where amount > 200.00 into a dataframe
customer_deposits_qry = spark.sql("select * from BankDeposits")
...
# TODO: using a select expression on the streaming dataframe, cast the key and the value columns from kafka as strings, and then select them
customer_df = customer_stream.selectExpr("cast(key as string) key", "cast(value as string) value")

# TODO: using the kafka message StructType, deserialize the JSON from the streaming dataframe 
# TODO: create a temporary streaming view called "BankCustomers" 
# it can later be queried with spark.sql
customer_df \
    .withColumn("value",from_json("value",customerKafkaMessageSchema)) \
    .select(col('value.*')) \
    .createOrReplaceTempView("BankCustomers")

# TODO: using spark.sql, select customerName, accountNumber as customerNumber from BankCustomers into a dataframe
customer_qry = spark.sql("select customerName, accountNumber as customerNumber from BankCustomers")

# TODO: join the customer dataframe with the deposit dataframe
join_expr = """accountNumber = customerNumber"""
customer_with_deposits_df = customer_deposits_qry.join(customer_qry, expr(join_expr))
...
#. +-------------+------+--------------------+------------+--------------+
customer_with_deposits_df.writeStream \
    .outputMode("append") \
    .format("console") \
    .start() \
    .awaitTermination()
